...
export default class GistCommand extends Command {
  constructor() {
    super('gist', {
      aliases: ['gist', 'paste'],
      channel: 'guild',
            ...
            .send({
              embeds: [
                new MessageEmbed(embed)
                  ...
                  .setTitle(
                    'Requested message was successfully pasted to gist!'
                  )
                  .setDescription(`**${gist.body.html_url}**`)
                  .setAuthor(
                    `Code by ${code.author.tag}`,
                    code.author.avatarURL({ dynamic: true })
                  )
